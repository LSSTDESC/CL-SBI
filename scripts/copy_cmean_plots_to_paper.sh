#!/bin/bash
# Copy the corrected-mean (IR2/ZC1) figures into tex_source/figures_new/, replacing the
# median-stack versions AT THE SAME PATHS so main.tex figure references are unchanged.
# A one-time backup of the median-stack versions is kept in tex_source/figures_new_median_backup/.
cd "$(dirname "$0")/.."
OUT=outputs/plots
FIG=tex_source/figures_new
BAK=tex_source/figures_new_median_backup

# one-time backup of the Sigma figure tree
if [ ! -d "$BAK" ]; then
    cp -R "$FIG" "$BAK" && echo "Backed up median-stack figures -> $BAK"
fi

copy() {  # $1 = output dir basename (without .10000.376), $2 = figures_new subdir
    src="$OUT/$1.10000.376.delta_sigma.corrected_mean"
    dst="$FIG/$2"
...
for f in aggregate_ftj_in_distro aggregate_ftj_ood_noise_mc aggregate_ftj_ood_rm; do
    [ -f "$OUT/infer_agg.delta_sigma.corrected_mean/$f.pdf" ] && cp "$OUT/infer_agg.delta_sigma.corrected_mean/$f.pdf" "$FIG/infer_agg/" && echo "  infer_agg/$f.pdf"
done

echo "Copying calibration overlays (Fig 13)..."
for f in calibration_ftj_overlay_10k_376 calibration_ftj_overlay_10k_376_ood; do
    [ -f "$OUT/calibration_overlay.delta_sigma.corrected_mean/$f.pdf" ] && cp "$OUT/calibration_overlay.delta_sigma.corrected_mean/$f.pdf" "$FIG/calibration/" && echo "  calibration/$f.pdf"
done
