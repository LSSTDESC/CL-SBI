...
in $\sim$2\,hours on a single CPU, after which each population posterior is a single forward pass.
Table~\ref{tab:afull} reports the recovered hyperparameters against truth for all six experiments.
...
We validate the network with simulation-based calibration (SBC) over the full envelope before
...
a falsely confident one --- erring, importantly, in the honest direction. Second, the present run
uses a single-Gaussian population in $\log_{10}M$ and therefore excludes the two richness-contamination
experiments, whose in-bin mass distribution is substantially non-Gaussian; extending the network with
a two-component mixture population is a direct refinement.
\todo{Add the larger-training-set rerun (tightening $\mu_M$, $\beta$ calibration); add the
two-component mixture run covering the richness-contamination experiments; and a direct
head-to-head figure against hierarchical HMC.}
