...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/ppc_neural_hbi.png}
\caption{Posterior predictive check for the neural HBI across all eight experiments. For each, a
population drawn from the inferred hyperparameters is forward-modelled (purple band: $16$--$84\%$;
purple line: median) and compared to the observed profiles (grey) and their median (black); the right
column shows the fractional residual. The predicted population reproduces the observed profiles in
every experiment, including the two richness-contamination cases (dashed, hatched), whose mass
\emph{spread} the single-Gaussian model still captures even though it mis-estimates their $M$--$c$
\emph{relation} (Table~\ref{tab:afull}).\label{fig:afull_ppc}}
\end{figure}

A posterior predictive check (Figure~\ref{fig:afull_ppc}) confirms that the population inferred by
the network, pushed back through the forward model, reproduces the observed stacked profiles in every
experiment. We validate the network with simulation-based calibration (SBC) over the full envelope before
...
case) and its predictive profiles remain consistent with the data (Figure~\ref{fig:afull_ppc}),
