...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.92\textwidth]{figures/three_method_sigmaM.png}
  \caption{Recovered population mass spread $\sigma_{\log_{10}M}$ vs.\ the true value (grey) for each
  experiment, across all methods. The single-$(M,c)$ MCMC joint-likelihood (blue) collapses to a
  near-zero spread (``DNC'' marks the one non-converged run), recovering a central value rather than
  the population width. SBI FTJ (orange) and all three hierarchical methods of this work---HMC
  (green), Hierarchical SBI (purple), and Hybrid SBI--HMC (brown)---recover the true spread,
  including the strongly out-of-distribution high $\lambda$--$M$ scatter case ($\sigma_M\approx0.54$)
  where MCMC fails entirely.}
  \label{fig:sigmaM}
\end{figure*}
...
recoverable quantities: the population mass spread $\sigma_M$ (Figure~\ref{fig:sigmaM}), the central
