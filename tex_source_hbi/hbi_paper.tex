...
\begin{figure}[t]
  ...
  \caption{Calibration of the inference approaches for the baseline experiment. The
  ...
  population until $p\to1$---whereas SBI FTJ (orange, $\Delta_{\max}=0.02$), the hierarchical
  HMC (green, $\Delta_{\max}=0.11$), and the fully-amortized neural HBI (purple, $\Delta_{\max}=0.05$;
  Section~\ref{sec:neural_hbi}) are all well calibrated.}
...
($\Delta_{\max}=0.93$), while the hierarchical HMC, SBI FTJ, and the fully-amortized neural HBI
introduced in Section~\ref{sec:neural_hbi} all recover near-nominal coverage
($\Delta_{\max}=0.11$, $0.02$, and $0.05$).
...
\caption{Simulation-based calibration of the fully-amortized neural HBI over the full training
envelope ($N=500$ held-out datasets). Uniform rank histograms indicate calibrated posteriors; the
grey band is the $99\%$ interval expected under uniformity. The population spread $\sigma_M$, the
$M$--$c$ normalization $c_0$, and the concentration scatter $\sigma_c$ are well calibrated; the mean
mass $\mu_M$ and the slope $\beta$ are mildly over-confident (Kolmogorov--Smirnov distances $0.119$
and $0.106$), a residual we expect to improve with a larger training set.\label{fig:afull_sbc}}
\end{figure}
...
its coverage is well calibrated (Figure~\ref{fig:calibration}, $\Delta_{\max}=0.05$, comparable to
