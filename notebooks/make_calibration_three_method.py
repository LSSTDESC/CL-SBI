...
# Neural HBI: draw a population from its inferred hyperparameter posterior (same construction as HMC),
# then the SAME compute_coverage. Uses the per-stack posterior samples saved in afull_neural_hbi.pkl.
...
ax.plot(x, y(cov_nhbi), color="#9467bd", lw=2.2, marker="D", ms=3, label=f"Neural HBI (this work), $\\Delta_{{\\max}}$={dmax(cov_nhbi):.2f}")
