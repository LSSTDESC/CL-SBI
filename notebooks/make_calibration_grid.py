...
for ax,(obs,title) in zip(axes,EXPS):
    ...
    ax.text(0.04,0.96,f"$\\Delta_{{\\max}}$: M={dmax(cov_mcmc):.2f} S={dmax(cov_sbi):.2f} H={dmax(cov_hmc):.2f}",
...
fig.suptitle("Calibration across experiments (fit-then-join): empirical coverage vs nominal credible level",fontsize=14)
